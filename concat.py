...
num_unique = lambda x: (
    ...
    >> wdotc("split")
    >> mk(lambda x: set(x))
    >> mk(  # hack with lambda, since the way we detect num arguments doesn't work for 'set' for some reason
        len
    )
)()
...
def inc_elem(a, b):
    ...
    pipe = (
        push(b)
        >> push(a)
        >> mk(lambda a, b: a.__getitem__(b))
        >> inc
        >> push(b)
        >> push(a)
        >> mk(lambda a, b, c: b.__setitem__(c, a))
        >> drop
        >> push(b)
    )
    return pipe()
...
def test_word_count():
    assert word_count("the second largest ocean is the second largest ocean") == (
        {"the": 2, "second": 2, "largest": 2, "ocean": 2, "is": 1},
    )
